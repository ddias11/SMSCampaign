		...
		public synchronized byte get() throws TimeoutException, InterruptedException {
			...
			while (true) {
				try {
					...
					if (this.bufferStart == this.bufferEnd) {
						if (this.buffer[this.bufferStart] != 0x0a)
							put((byte) 0x0a);
						c = this.buffer[this.bufferStart + 1];
						// throw new
						// TimeoutException("No response from device.");
					} else {
	...
	protected void connect() throws TimeoutException, GatewayException, IOException, InterruptedException {
		...
		synchronized (getSYNCCommander()) {
			try {
				...
				setCnmiEmulationProcessor(new CNMIEmulatorProcessor("CNMIEmulatorProcessor [" + getGateway().getGatewayId() + "]",
